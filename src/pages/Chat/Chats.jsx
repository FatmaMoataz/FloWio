...
import {
  FaSearch, FaPhoneAlt, FaEllipsisH, FaPaperPlane, FaSmile, FaPaperclip, FaPlus, FaUsers,
  FaUserTie, FaUserAstronaut, FaUserNinja, FaUserGraduate, FaTrash, FaInfoCircle, FaBellSlash,
  FaFileAlt, FaImage, FaVideo, FaMicrophone, FaTimes, FaThumbtack,
} from "react-icons/fa";
...
const initialRooms = [
  { id: "room-1", name: "FloWio Group Chat", msg: "Tap to view history", time: "Now", online: true, pinned: true, icon: <FaUsers />, color: "from-[#6eb5ff] to-[#5b7dff]" },
  { id: "room-2", name: "General Room", msg: "Start talking here", time: "Now", online: true, pinned: false, icon: <FaUserTie />, color: "from-[#ffb86b] to-[#ff7b54]" }
];
...
function Avatar({ icon, online, color, small }) {
  return (
    <div className={`relative flex shrink-0 items-center justify-center rounded-full bg-gradient-to-b ${color || defaultColor} text-white shadow-[0_0_18px_rgba(255,255,255,.10)] ${small ? "h-10 w-10 text-sm" : "h-12 w-12 text-lg"}`}>
      {icon || defaultIcon}
      <span className={`absolute right-0 top-0 rounded-full border-2 border-[#0c144a] ${online ? "bg-[#37e783]" : "bg-white/70"} ${small ? "h-3 w-3" : "h-3.5 w-3.5"}`} />
    </div>
  );
}
...
export default function Chats() {
  ...
  useEffect(() => {
    ...
    const fetchChatHistory = async () => {
      try {
        const response = await axios.get(`${BACKEND_URL}/api/messages/${activeChat.name}`);
        
        const formattedMessages = response.data.map(msg => ({
          from: msg.sender?._id === CURRENT_USER_ID || msg.sender === CURRENT_USER_ID ? "me" : "other",
          text: msg.text,
  ...
  const clearChat = () => { setMessages([]); setMenuOpen(false); };
  ...
  const ChatCard = ({ chat }) => (
    <button
      onClick={() => { setActiveChat(chat); setMenuOpen(false); setNewChatOpen(false); }}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1">
            ...
            {chat.pinned && <FaThumbtack className="text-[9px] text-[#6eb5ff]" />}
          </div>
        ...
        <div className="min-h-0 overflow-y-auto rounded-[28px] border border-white/5 bg-gradient-to-br from-[#151e66]/95 to-[#0c123f]/95 p-5 shadow-[0_22px_55px_rgba(0,0,0,.30)]">
          <div className="mb-5 flex items-center justify-between">
            ...
            <div className="relative">
              <button onClick={() => setNewChatOpen((prev) => !prev)} className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-r from-[#6eb5ff] to-[#5b7dff] text-sm shadow-[0_0_18px_rgba(95,150,255,.35)] transition hover:brightness-110"><FaPlus /></button>
              {newChatOpen && <NewChatOverlay onClose={() => setNewChatOpen(false)} onCreate={createNewChat} />}
            </div>
          </div>
          ...
          <div className="mb-5 flex h-11 items-center rounded-[16px] border border-blue-300/10 bg-[#141d66]/90 px-4">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search..." className="ml-3 w-full bg-transparent text-xs text-white outline-none placeholder:text-white/35" />
          </div>

          <p className="mb-3 text-[12px] text-white/45">Pinned Chat</p>
          <div className="space-y-3">
            {pinnedChats.length > 0 ? pinnedChats.map((chat) => <ChatCard key={chat.id} chat={chat} />) : <p className="rounded-2xl bg-[#10184c]/60 p-3 text-[11px] text-white/35">No pinned chats yet.</p>}
          </div>

          <p className="mb-3 mt-6 text-[12px] text-white/45">All Chats</p>
          <div className="space-y-3">
            {allChats.length > 0 ? allChats.map((chat) => <ChatCard key={chat.id} chat={chat} />) : <p className="rounded-2xl bg-[#10184c]/60 p-3 text-[11px] text-white/35">No chats available.</p>}
          </div>
        </div>
          ...
          {activeChat ? (
            <>
              <div className="relative flex shrink-0 items-center justify-between border-b border-white/10 px-6 py-4">
                <div className="flex items-center gap-3">
                  <Avatar icon={activeChat.icon} online={activeChat.online} color={activeChat.color} />
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-[16px] font-bold">{activeChat.name}</h3>
                      {activeChat.pinned && <FaThumbtack className="text-[11px] text-[#6eb5ff]" />}
                    </div>
                    <p className="mt-1 text-[11px] text-[#37e783]">{activeChat.online ? "Active Now" : "Offline"}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <button onClick={() => setMenuOpen((prev) => !prev)} className="text-white/75 transition hover:text-[#6eb5ff]"><FaEllipsisH /></button>
                </div>
              </div>

              {/* MESSAGES AREA */}
              <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-5">
                {messages.length === 0 && <div className="flex h-full items-center justify-center text-sm text-white/40">No messages yet.</div>}
                {messages.map((msg, index) => (
                  <div key={index} className={`flex items-end gap-3 ${msg.from === "me" ? "justify-end" : "justify-start"}`}>
                    ...
                    <div className={`max-w-[430px] ${msg.from === "me" ? "text-right" : "text-left"}`}>
                      ...
                      <div className={`mt-2 flex items-center gap-2 text-[10px] text-white/45 ${msg.from === "me" ? "justify-end" : ""}`}>
                        <span>{msg.time}</span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="relative shrink-0 border-t border-white/10 p-4">
                <div className="flex h-12 items-center gap-3 rounded-[16px] border border-blue-300/10 bg-[#141d66]/90 px-4">
                  <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} placeholder="Write messages..." className="flex-1 bg-transparent text-xs text-white outline-none placeholder:text-white/35" />
                  <button onClick={sendMessage} className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-r from-[#6eb5ff] to-[#5b7dff] text-sm shadow-[0_0_18px_rgba(95,150,255,.35)] transition hover:brightness-110"><FaPaperPlane /></button>
                </div>
              </div>
            </>
          ) : (
            <div className="flex h-full items-center justify-center text-white/40">Select a chat to start messaging</div>
          )}
        </div>
      </div>
    </MainLayout>
