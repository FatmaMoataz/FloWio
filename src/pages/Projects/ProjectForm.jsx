...
import { FaArrowLeft, FaBookOpen, FaFolderPlus, FaPlus, FaTrashAlt, FaSpinner } from "react-icons/fa";
...
const makeStory = () => ({ tempId: crypto.randomUUID(), name: "", subtasks: [makeSubtask()] });
...
export default function ProjectForm() {
  ...
  const [form, setForm] = useState({ name: "", description: "", startDate: new Date().toISOString().split('T')[0], endDate: "", owner: "", priority: "Medium", epics: [makeEpic()] });
  ...
  const updateStory = (epicTempId, storyTempId, value) => setForm((c) => ({ ...c, epics: c.epics.map((epic) => epic.tempId === epicTempId ? { ...epic, stories: epic.stories.map((story) => story.tempId === storyTempId ? { ...story, name: value } : story) } : epic) }));
  ...
  const handleSubmit = async (event) => {
    ...
    setError("");
    const cleanEpics = form.epics.filter((e) => e.name.trim()).map((e) => ({ name: e.name.trim(), progress: e.progress || 0, stories: e.stories.filter((s) => s.name.trim()).map((s) => ({ name: s.name.trim(), subtasks: s.subtasks.filter((st) => st.name.trim()).map((st) => ({ name: st.name.trim() })) })) }));
    ...
    try {
      ...
      const projectRes = await projectService.createProject({ name: form.name.trim(), description: form.description.trim(), companyId, status: "active", startDate: form.startDate, endDate: form.endDate });
      ...
      for (const epic of cleanEpics) {
        try {
          const epicRes = await API.post("/api/epics", { name: epic.name, description: `${epic.stories.length} stories planned`, status: epic.progress >= 100 ? "Done" : epic.progress > 0 ? "In Progress" : "To Do", companyId, projectId });
          ...
          for (let i = 0; i < epic.stories.length; i++) {
            ...
            try {
              const storyRes = await storyService.createStory({ title: story.name, description: `Story in epic: ${epic.name}`, status: "To Do", priority: form.priority, epicId, projectId, companyId, dueDate: form.endDate, order: i });
              if (!storyRes.success) continue;
              const storyId = storyRes.data._id;
              for (const subtask of story.subtasks) {
                try { await API.post("/api/subtasks", { title: subtask.name, description: `Subtask for story: ${story.name}`, status: "To Do", storyId, companyId, due_date: form.endDate }); }
        ...
        <form onSubmit={handleSubmit} className="mx-auto mt-5 max-w-4xl">
          <div className="flowio-project-info-card rounded-[26px] border border-white/[0.06] bg-[#111846]/80 p-5 shadow-xl sm:p-8">
            ...
            <div className="mt-7 grid gap-5 sm:grid-cols-2">
              ...
              <label><span className="mb-2 block text-xs font-medium text-white/65">Project owner</span><input name="owner" value={form.owner} onChange={updateField} placeholder="Team member name" className={inputClass} /></label>
                ...
                <div className="mt-5 space-y-4">
                  {epic.stories.map((story, storyIndex) => (
                    <div key={story.tempId} className="flowio-project-story-editor rounded-2xl border border-[#34458a]/60 bg-[#090f37]/75 p-4">
                      <div className="flex items-center gap-3"><span className="shrink-0 text-xs font-medium text-[#9bbdff]">Story {storyIndex + 1}</span><input value={story.name} onChange={(e) => updateStory(epic.tempId, story.tempId, e.target.value)} placeholder="Story name" className={inputClass} />{epic.stories.length > 1 && <button type="button" onClick={() => removeStory(epic.tempId, story.tempId)} className="shrink-0 rounded-lg p-2 text-rose-300/60 hover:bg-rose-500/10 hover:text-rose-300"><FaTrashAlt /></button>}</div>
