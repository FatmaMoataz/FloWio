...
import { FaLaptopCode, FaRobot, FaPalette, FaProjectDiagram, FaPlus, FaTimes } from "react-icons/fa";
...
export default function Projects() {
  ...
  const handleCreateProject = async (e) => {
    ...
    try {
      ...
      const response = await fetch("https://flowio-backend.vercel.app/api/projects", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-auth-token": token,
      ...
      setFormError(err.message || "Something went wrong while creating the project.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <MainLayout title="Projects">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-white/50 mt-1">Manage and track your operational pipeline</p>
        </div>
        <button
      ...
      {!loading && !error && projects.length === 0 && (
        <div className="flex h-64 flex-col items-center justify-center text-white/50 bg-[#111b63]/40 rounded-[24px] border border-white/5 p-6 text-center">
          <p className="mb-4">No projects found for this company. Create one to get started!</p>
          <button
            onClick={() => setIsModalOpen(true)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="w-full max-w-md overflow-hidden rounded-[28px] border border-white/10 bg-gradient-to-br from-[#16206d] to-[#0d1448] p-6 shadow-2xl text-white">
            ...
            <form onSubmit={handleCreateProject} className="mt-4 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-white/70 mb-1.5">Project Name *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-white/70 mb-1.5">Description</label>
                <textarea
                  rows="3"
